...
class NotionPageScreen extends StatefulWidget {
  // ★ 1. 페이지 ID를 필수로 받습니다.
  final String pageId; 
  const NotionPageScreen({required this.pageId, super.key}); 
...
class _NotionPageScreenState extends State<NotionPageScreen> {
  ...
  void initState() {
    ...
    _currentBlocks = getPageBlocks(widget.pageId); 
    ...
    try {
      ...
      if (pickedFile != null) {
        setState(() {
          ...
          _currentBlocks.insert(1, BlockData(type: 'image', content: pickedFile.path));
        });
      }
    } catch (e) {
      ...
      builder: (context) {
        return Container(
          ...
          child: Column(
            ...
            children: [
              const Text("이미지 추가", style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
        ...
        builder: (BuildContext context) {
          return TableSizeSelectorModal(
            onTableCreated: (rows, cols) {
              setState(() {
                _currentBlocks.insert(1, BlockData(type: 'table', content: {'rows': rows, 'cols': cols}));
              });
        ...
        itemBuilder: (context, index) {
          ...
          switch (block.type) {
            ...
            case 'table':
              ...
              return NotionTable(rows: size['rows']!, cols: size['cols']!, key: ValueKey(block));
            case 'image':
