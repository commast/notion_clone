...
class _ChartEditModalState extends State<ChartEditModal> {
  ...
  void initState() {
    super.initState();
    _editingData = widget.initialData.map((item) => Map<String, dynamic>.from(item)).toList();
  ...
  void _initializeControllers() {
    ...
    for (var item in _editingData) {
      _labelControllers.add(TextEditingController(text: item['label'] as String));
  ...
  void _addItem() {
    setState(() {
      ...
      _editingData.add({'label': 'New Item', 'value': 50.0, 'color': notionColors[newColorIndex]});
      _initializeControllers();
    });
  }
      ...
      child: Column(
        ...
        children: [
          Row(
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
            children: [
              const Text('막대 차트 데이터 편집', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
          ...
          const SizedBox(height: 10),
          TextButton.icon(onPressed: _addItem, icon: const Icon(Icons.add, color: Colors.blue), label: const Text('항목 추가', style: TextStyle(color: Colors.blue, fontSize: 16))),
          const SizedBox(height: 15),
          SizedBox(
            width: double.infinity,
                  ...
                  return PopupMenuItem<Color>(
                    ...
                    child: Row(
                      children: [
                        ...
                        Text(color.value == data['color'].value ? '선택됨' : '선택', style: TextStyle(color: color.value == data['color'].value ? Colors.blue : Colors.black)),
                      ],
                    ),
                  );
                }).toList(),
